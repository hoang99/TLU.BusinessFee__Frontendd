...
class QuanLyPhongBan extends Component {
  ...
  onThemMoi = () => {
    ...
    axios({
      ...
      .then(
        (res) =>
          this.props.alertOn_TrangThaiThemMoiThanhCong(
            res
  ...
  onSua = () => {
    if (window.confirm("Bạn có chắc chắn muốn sửa")) {
      const dataEdit = {
        ...
        tenPhongBan: this.state.tenPhongBan
          ? this.state.tenPhongBan
          : this.state.hienThiSuaUer?.tenPhongBan,
        ngayThanhLap: this.state.ngayThanhLap
          ? this.state.ngayThanhLap
          : this.state.hienThiSuaUer?.ngayThanhLap,
      };
      const indexDataEdit = this.state.data.findIndex(
        (e) => e.maPhongBan === this.state.hienThiSuaUer.maPhongBan
      );
      this.state.data.splice(indexDataEdit, 1, dataEdit);
      axios({
        method: "PUT",
        url:
          "https://localhost:5001/api/PhongBan/" +
          this.state.hienThiSuaUer?.maPhongBan,
        headers: {
          Authorization: "Bearer " + localStorage.getItem("token"),
        },
        data: dataEdit,
        ...
        .then((res) => res.data)
        .then((res) => {
          this.props.alertOn_TrangThaiSuaThanhCong(res);
        })
  ...
  onDelete = (maPhongBan) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa")) {
      ...
      var tempData = this.state.data.filter(
        (item) => item.maPhongBan !== maPhongBan
      );
        ...
        <td>
                        ...
                        <div className="form-group">
                          <div className="form-group">
                            ...
                            <input
                              ...
                              name="tenPhongBan"
                              Value={this.state.hienThiSuaUer.tenPhongBan}
                              onChange={(event) => this.onChange(event)}
                            />
                            ...
                            <input
                              type="date"
                              className="form-control"
                              placeholder="Ngày thành lập (DD-MM-YYYY)"
                              name="ngayThanhLap"
                              Value={this.state.hienThiSuaUer.ngayThanhLap}
                              onChange={(event) => this.onChange(event)}
                            />
                          </div>
                        </div>
          ...
          <div className="btn btn-danger btn-group ml-2">
            <div
              ...
              onClick={() => this.onDelete(value.maPhongBan)}
            >
              Xóa
            </div>
          </div>
        ...
        <div class="row-container ">
          ...
          <div className="table-manager" style={{ padding: "0 25px 0 0" }}>
            <div className="heading-manager">
              <i class="fas fa-home"></i> Quản lý phòng ban
            </div>
            <div className="">
              {/* begin search */}
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    alertOn_TrangThaiThemMoiThanhCong: () => {
      dispatch(actions.alertOn_TrangThaiThemMoiThanhCong(), 2000);
    },
