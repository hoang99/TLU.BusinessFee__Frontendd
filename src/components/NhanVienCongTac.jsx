...
class NhanVienCongTac extends Component {
  ...
  hienThiTenPhongBan = () => {
    return this.state.dataPhongBan.map((value) => (
      <option>{value.tenPhongBan}</option>
    ));
  };
  hienThiTenCapBac = () => {
    return this.state.dataCapBac.map((value) => (
  ...
  hienThiTenNhanVien = () => {
    ...
    this.state.dataNhanVien.forEach((item) => {
      if (
        item.tenPhongBan == this.state.getOnChangePhongBan &&
        item.tenCapBac == this.state.getonChangeCapBac
      ) {
    ...
    return getDaTaNhanVienTheoCapBacVaPhongBan.map((value) => (
  ...
  onDelete = (maChuyenCongTac, maNhanVien) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa")) {
      var tempData = this.state.data.filter(
        (item) =>
          item.maChuyenCongTac !== maChuyenCongTac ||
      ...
      axios({
        method: "DELETE",
        url:
          "https://localhost:5001/api/NhanVienCongTac?MaChuyenCongTac=" +
          maChuyenCongTac +
          "&MaNhanVien=" +
          maNhanVien,
        headers: {
          ...
          {value.trangThai === "Chua thuc hien" ? (
            <div className="btn btn-danger btn-group">
              <div
                ...
                onClick={() =>
                  this.onDelete(value.maChuyenCongTac, value.maNhanVien)
                }
              >
                {" "}
                Xóa
        ...
        <div class="row-container">
          ...
          <div className="table-manager" style={{ padding: "0 25px 0 0" }}>
            <div className="heading-manager">
              <i></i> Quản lý nhân viên công tác
            </div>
            <div className="">
              {/* begin search */}
                          ...
                          <div className="form-group">
                            <div className="form-group">
                              ...
                              <select
                                ...
                                aria-label="Default select example"
                                name="tenCapBac"
                                onChange={(value) => this.onChangeCapBac(value)}
                              >
